...
建议采用分阶段后训练以避免理解/生成互相干扰：

1. **阶段A：LVR SFT（理解优先）**
   - 仅训练 `lvr_head` 与少量连接层，保持 LLM/ViT 大部分冻结。
   - 数据：LVR 格式（含 `<lvr>` 和 bbox）为主，混少量 VLM。
2. **阶段B：CoVT Anchor 对齐（感知补强）**
   - 使用 `<|sam_pad|> / <|depth_pad|> ...` 等 anchor tokens。
   - 主要训练 anchor 投影层/查询向量（如使用预计算 teacher 特征）。
3. **阶段C：多任务融合**
   - 解冻部分 LLM block 与 ViT connector，联合训练 T2I/Editing/VLM + LVR/CoVT。
   - 逐步提高生成数据权重以恢复生成质量。
4. **阶段D：可选 GRPO_latent 强化**
   - 使用 LVR 的 GRPO 思路做小步数 RL，仅更新少量参数。

关键训练参数示例：
```bash
--lvr_head True --lvr_head_type simple --lvr_weight 1.0 \
--anchor_weight 1.0 --lvr_loss_fct mse --anchor_loss_fct mse
```
