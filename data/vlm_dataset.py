    ...
    def change_format(self, data, num_images):
        ...
        for conversation in data['conversations']:
            if conversation['from'] == 'human':
                ...
                if '<lvr>' in conversation['value']:
                    parts = conversation['value'].split('<lvr>')
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for row_idx, (data, image_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                ...
                lvr_local_vit_indexes = []
                lvr_bboxes = data_item.get('bboxes', []) or []
                if lvr_bboxes and image_tensor_list:
                    ref_image = image_tensor_list[0]
                    _, img_h, img_w = ref_image.shape
                    patch_size = transform_stride
                    num_patches_w = max(img_w // patch_size, 1)
                    num_patches_h = max(img_h // patch_size, 1)
                    for bbox in lvr_bboxes:
                        ...
                        lvr_local_vit_indexes.append(py * num_patches_w + px)
                ...
                yield dict(
                    ...
                    lvr_local_vit_indexes=lvr_local_vit_indexes,
                    data_indexes={
