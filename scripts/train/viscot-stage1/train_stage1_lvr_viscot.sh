...
EXPECTED_NUM_TOKENS=${EXPECTED_NUM_TOKENS:-5120}          # 4卡总计 5.12K tokens (每卡约1.28K，大幅降低显存)
MAX_NUM_TOKENS=${MAX_NUM_TOKENS:-6144}                    # 允许弹性上限 +20%
MAX_NUM_TOKENS_PER_SAMPLE=${MAX_NUM_TOKENS_PER_SAMPLE:-6144}  # 单样本最大 6K


# LVR 配置
LVR_HEAD=${LVR_HEAD:-False}                            # 是否使用 LVR head
LVR_HEAD_TYPE=${LVR_HEAD_TYPE:-simple}                 # LVR head 类型: simple/glu
LVR_WEIGHT=${LVR_WEIGHT:-0.1}                          # LVR 主损失权重
LVR_LOSS_FCT=${LVR_LOSS_FCT:-mse}                      # LVR 损失函数: mse/mae/cosine
LVR_LATENT_END_TOKEN=${LVR_LATENT_END_TOKEN:-False}    # 是否启用退出策略（会自动覆盖 YAML 配置）
LVR_LATENT_END_WEIGHT=${LVR_LATENT_END_WEIGHT:-0.1}    # LVR 退出策略损失权重  
...
CPU_OFFLOAD=${CPU_OFFLOAD:-False}                          # 启用CPU offload以节省显存  

NUM_SHARD=${NUM_SHARD:-$nproc_per_node}                   # FSDP分片数，应与GPU数一致
...
if [ ! -d "$PARQUET_DIR" ] || [ -z "$(ls -A $PARQUET_DIR 2>/dev/null)" ]; then
    ...
    echo "  bash scripts/train/viscot-stage1/prepare_viscot_stage1.sh"
    exit 1
fi
...
echo "=========================================="
echo "CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES:-auto (SLURM managed)}"
...
echo "  Max tokens/sample: $MAX_NUM_TOKENS_PER_SAMPLE"
GRADIENT_ACCUMULATION=${GRADIENT_ACCUMULATION:-6}        
...
echo "  Num workers: $NUM_WORKERS (每GPU: ~$((NUM_WORKERS/NUM_GPUS)) workers)"
echo "  Prefetch factor: $PREFETCH_FACTOR"
echo "  CPUs allocated: ${SLURM_CPUS_PER_TASK:-N/A} (workers使用: $NUM_WORKERS)"
echo "----------------------------------------"
echo "LVR Stage1 config:"
echo "  LVR HEAD: $LVR_HEAD "
echo "  LVR HEAD type: $LVR_HEAD_TYPE"
echo "  LVR weight: $LVR_WEIGHT"
echo "  LVR loss function: $LVR_LOSS_FCT"
echo "  LVR latent end token: $LVR_LATENT_END_TOKEN"
echo "  LVR latent end weight: $LVR_LATENT_END_WEIGHT"
echo "  Freeze strategy: Vision=True, Merger=True, LLM=False"
echo "----------------------------------------"
