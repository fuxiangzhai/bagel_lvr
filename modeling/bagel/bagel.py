...
class BagelConfig(PretrainedConfig):
    def __init__(
        ...
        lvr_start_id=None,
        lvr_end_id=None,
        lvr_latent_end_id=None,
        lvr_latent_end_token=False,
        ...
        **kwargs
        ...
        self.lvr_head = lvr_head
        self.lvr_head_type = lvr_head_type
        self.lvr_loss_fct = lvr_loss_fct
        self.lvr_token_id = lvr_token_id
        self.lvr_start_id = lvr_start_id
        self.lvr_end_id = lvr_end_id
        self.lvr_latent_end_id = lvr_latent_end_id
        self.lvr_latent_end_token = lvr_latent_end_token
        self.anchor_token_ids = anchor_token_ids
        self.anchor_dims = anchor_dims
        self.anchor_loss_fct = anchor_loss_fct
...
class Bagel(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, config: BagelConfig):
        ...
        self.lvr_head = None
        if config.lvr_head:
            if config.lvr_head_type == "glu":
                self.lvr_head = LVRHeadGLU(
                    hidden_size=self.hidden_size,
                    intermediate_size=config.llm_config.intermediate_size,
                    hidden_act=config.llm_config.hidden_act,
                )
            else:
                self.lvr_head = LVRHead(hidden_size=self.hidden_size)

        if config.lvr_latent_end_token:
            target_norm_scale = 1.0
            with torch.no_grad():
                v = torch.randn(self.hidden_size)
                v = v / (v.norm() + 1e-6)
                v = v * (target_norm_scale * (self.hidden_size ** 0.5))
            self.lvr_latent_end_emb = nn.Parameter(v)

        self.anchor_token_ids = config.anchor_token_ids or {}
        default_anchor_dims = {
            "sam": 256,
            "dino": 1024,
            "depth": 1024,
        ...
        anchor_dims = config.anchor_dims or default_anchor_dims
        self.anchor_projections = nn.ModuleDict(
            {name: nn.Linear(self.hidden_size, dim) for name, dim in anchor_dims.items()}
        )
    ...
    def _embedding_loss(self, pred: torch.Tensor, target: torch.Tensor, loss_fct: str) -> torch.Tensor:
        ...
        raise ValueError(f"Unsupported embedding loss: {loss_fct}")
    ...
    def forward(
        ...
        if anchor_target_embeddings:
            anchor_losses = []
            for name, target in anchor_target_embeddings.items():
                token_id = self.anchor_token_ids.get(name)
                if token_id is None:
                    continue
                anchor_mask = packed_text_ids == token_id
                if not anchor_mask.any():
                    continue
                anchor_token_indexes = packed_text_indexes[anchor_mask]
                if anchor_token_indexes.numel() == 0:
                    continue
                pred_anchor = last_hidden_state[anchor_token_indexes]
                if name in self.anchor_projections:
                    pred_anchor = self.anchor_projections[name](pred_anchor)
                target = target.to(pred_anchor.device, pred_anchor.dtype)
                anchor_count = min(pred_anchor.shape[0], target.shape[0])
                if anchor_count <= 0:
                    continue
                anchor_loss = self._embedding_loss(
                    pred_anchor[:anchor_count],
                    target[:anchor_count],
                    self.config.anchor_loss_fct,
                )
