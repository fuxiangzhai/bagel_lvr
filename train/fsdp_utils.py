...
def fsdp_wrapper(original_model, fsdp_config, ignored_modules=[]):
    ...
    return FSDP(
        ...
        use_orig_params=True,  # 允许混合 requires_grad 状态（冻结+训练）
    )
    ...
    def fsdp_save_ckpt(
        ...
        if ema_model is not None:
            with FSDP.state_dict_type(
                ...
                if dist.get_rank() == 0:
                    # 转换为 BF16 后保存
                    ema_state_dict_bf16 = {k: v.to(save_dtype) for k, v in ema_state_dict.items()}
                    save_file(ema_state_dict_bf16, os.path.join(save_path, "ema.safetensors"))
                    del ema_state_dict_bf16
        ...
        with FSDP.state_dict_type(
            ...
            if dist.get_rank() == 0:
                # 转换为 BF16 后保存
                model_state_dict_bf16 = {k: v.to(save_dtype) for k, v in model_state_dict.items()}
                save_file(model_state_dict_bf16, os.path.join(save_path, "model.safetensors"))
                del model_state_dict_bf16
