...
class ModelArguments:
    ...
    lvr_head: bool = field(
        default=False,
        metadata={"help": "Enable LVR head for latent visual reasoning supervision."}
    )
    lvr_head_type: str = field(
        default="simple",
        metadata={"help": "Type of LVR head: 'simple' or 'glu'."}
    )
    lvr_latent_end_token: bool = field(
        default=False,
        metadata={"help": "Enable learnable latent end embedding for LVR decoding."}
    )
...
class TrainingArguments:
    ...
    lvr_loss_fct: str = field(
        default="mse",
        metadata={"help": "LVR loss function: mse, mae, cosine."}
    )
    anchor_loss_fct: str = field(
        default="mse",
        metadata={"help": "Anchor loss function: mse, mae, cosine."}
    )
...
def main():
    ...
    config = BagelConfig(
        ...
        lvr_head_type=model_args.lvr_head_type,
        lvr_loss_fct=training_args.lvr_loss_fct,
        lvr_latent_end_token=model_args.lvr_latent_end_token,
        anchor_loss_fct=training_args.anchor_loss_fct,
    )
    model = Bagel(
        language_model, 
    ...
    model.config.lvr_token_id = new_token_ids.get("lvr_token_id")
    model.config.lvr_start_id = new_token_ids.get("lvr_start_id")
    model.config.lvr_end_id = new_token_ids.get("lvr_end_id")
    model.config.lvr_latent_end_id = new_token_ids.get("lvr_latent_end_id")
    model.config.lvr_loss_fct = training_args.lvr_loss_fct
    model.config.anchor_token_ids = {
        "sam": new_token_ids.get("sam_token_id"),
        "dino": new_token_ids.get("dino_token_id"),
        "depth": new_token_ids.get("depth_token_id"),
        "sd": new_token_ids.get("sd_token_id"),
        "intern": new_token_ids.get("intern_token_id"),
        "pidinet": new_token_ids.get("pidinet_token_id"),
        "siglip": new_token_ids.get("siglip_token_id"),
        "metaclip": new_token_ids.get("metaclip_token_id"),
    }
    # When finetune_from_hf=True, the checkpoint may have a larger vocab_size than the tokenizer
    # (because the checkpoint already includes special tokens in its embeddings).
    # We need to resize to match the checkpoint's vocab_size, not just len(tokenizer).
    target_vocab_size = len(tokenizer)
    if training_args.finetune_from_hf:
        # Use the vocab_size from llm_config.json (which matches the checkpoint)
        target_vocab_size = max(target_vocab_size, llm_config.vocab_size)
    
    current_vocab_size = model.language_model.config.vocab_size
    if target_vocab_size != current_vocab_size:
        logger.info(f"Resizing token embeddings: {current_vocab_size} -> {target_vocab_size}")
